...
import copy
import ctypes
from TwoLevelWeightVirtualization import VNN
from TwoLevelWeightVirtualization import TwoLevelWeightVirtualization

tf.logging.set_verbosity(tf.logging.ERROR)

tlwv_op = tf.load_op_library('./tf_operation.so')
...
def init_virtualization(tlwv, vnn_list):
	with tf.Graph().as_default() as graph:
		with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:

			time1 = time.time()
			shared_weight_address = sess.run(tlwv_op.init_weight(tlwv.weight_page))
			time2 = time.time()
			...
			for vnn in vnn_list:
				...
				time1 = time.time()
				page_address = sess.run(tlwv_op.init_page_table(page_list))
				time2 = time.time()
				...
				for i in vnn.exclusive_weight_page_list:
					exclusive_weight_page.extend(tlwv.exclusive_weight_page[(vnn.name,i)])
...
def virtualized_execution(tlwv, vnn, shared_weight_address, page_address, exclusive_weight_page):
	...
	with tf.Graph().as_default() as graph:
		with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
			...
			weight_address, weight_len = sess.run(tlwv_op.get_weight_address(train_weights))

			time1 = time.time()
			load_weight_page(shared_weight_address, weight_address,
				weight_len, page_address, exclusive_weight_page,
				tlwv.weight_per_page, tlwv.num_of_weight_page)
			time2 = time.time()
...
def main():
	...
	shared_weight_address, page_address_list, exclusive_weight_page_list \
		= init_virtualization(tlwv, vnn_list)
	...
	for i in range(num_execution):
		...
		weight_load_time, execution_time, reuslt, accuracy = virtualized_execution(tlwv, vnn,
