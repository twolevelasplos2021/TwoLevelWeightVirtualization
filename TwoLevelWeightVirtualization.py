...
class TwoLevelWeightVirtualization:
	__instance = None
	@staticmethod
	def getInstance():
		if TwoLevelWeightVirtualization.__instance == None:
			TwoLevelWeightVirtualization()
		return TwoLevelWeightVirtualization.__instance
	...
	def __init__(self, num_of_weight_page=128, weight_per_page=512,
		...
		if TwoLevelWeightVirtualization.__instance != None:
			...
			TwoLevelWeightVirtualization.__instance = self
	...
	def add_vnn(self, network_path, alpha, target_accuracy, margin,
		...
		# get virtual performance rate (vpr)
		if use_exclusive_page == 1:
			vpr = self.get_virtual_performance_rate(vnn, alpha, transpose,
		...
		self.get_essentinal_weight_page_list(vnn, vpr, margin)
	...
	def get_virtual_performance_rate(self, vnn, alpha, transpose, target_accuracy=None):
		...
		if target_accuracy is None:
			...
			vpr = np.maximum(0, 1 - (target_loss - loss) / loss)
		return vpr

	def get_essentinal_weight_page_list(self, vnn, vpr, margin):
...
def main(args):
	tlwv = TwoLevelWeightVirtualization()

	if args.mode == 'l':
		print('[VNN list]')
		for vnn_name in tlwv.vnns:
			vnn = tlwv.vnns[vnn_name]
		...
		weight_page_talbe = tlwv.load_weight_page_talbe()
		...
		tlwv.add_vnn(args.network_path, args.alpha, args.target_accuracy,
		...
		vnn = tlwv.vnns[args.vnn_name]
		tlwv.train_vnn(vnn, args.iter, args.transpose)
		tlwv.save_weight(vnn, args.transpose)
		...
		result, accuracy = tlwv.execute_vnn(vnn, input_variables,
		...
		vnn = tlwv.vnns[args.vnn_name]
		fisher_information = tlwv.compute_fisher(vnn, 'vnn')
		tlwv.save_vnn_fisher(vnn, fisher_information, args.transpose)
		...
		total_cost = tlwv.calculate_network_cost(vnn_list, args.transpose)
		...
		vnn = tlwv.vnns[args.vnn_name]
		layer = tlwv.vnn_layer(vnn, args.transpose)
		print(layer)

	elif args.mode == 'w':
		weight_vector = tlwv.get_weight_vector()
		...
		for name, vnn in tlwv.vnns.items():
			...
			for page_no in page_list:
				for exclusive_page in tlwv.exclusive_weight_page:
					if exclusive_page[0] == vnn.name and exclusive_page[1] == page_no:
						vnn_exclusive_weight.append(tlwv.exclusive_weight_page[exclusive_page])

			vnn_exclusive_weight = tlwv.vectorize_list(vnn_exclusive_weight)
		...
		for name, vnn in tlwv.vnns.items():
			...
			for exclusive_page in exclusive_page_list:
				page_list.insert(exclusive_page, tlwv.num_of_weight_page + idx)
				idx += 1
		...
		vnn = tlwv.vnns[args.vnn_name]
		target_loss = tlwv.compute_target_loss(vnn, args.target_accuracy, args.alpha,
		...
		vnn = tlwv.vnns[args.vnn_name]
		loss = tlwv.compute_loss(vnn, args.alpha, target='network', transpose=args.transpose)
		print('loss:', loss)
		...
		vnn = tlwv.vnns[args.vnn_name]
		vpr = tlwv.get_virtual_performance_rate(vnn, args.alpha, args.transpose,
			target_accuracy=args.target_accuracy)
		print('Virtual performance rate:', vpr)
		...
		vnn = tlwv.vnns[args.vnn_name]
		variables = tlwv.print_vnn_variable(vnn)
		...
		if args.vnn_name is None:
			...
			for name in vnn_name_list:
				...
				assert os.path.exists(vnn.network_path)
				vnn_list.append(vnn)
		tlwv.plot_network_fisher(vnn_list, args.transpose)
		...
		if args.vnn_name is None:
			...
			for name in vnn_name_list:
				...
				assert os.path.exists(vnn.network_path)
				vnn_list.append(vnn)
		tlwv.plot_network_weight(vnn_list, args.transpose)
		...
		vnn = tlwv.vnns[args.vnn_name]
		tlwv.plot_network_fisher_histogram(vnn, args.transpose)
		...
		vnn = tlwv.vnns[args.vnn_name]
		tlwv.plot_network_weight_histogram(vnn, args.transpose)
		...
		if args.vnn_name is None:
			...
			for name in vnn_name_list:
				...
				assert os.path.exists(vnn.network_path)
				vnn_list.append(vnn)
		tlwv.plot_sharing_cost_heatmap(vnn_list)
